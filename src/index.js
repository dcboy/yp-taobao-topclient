...
class TopClient {
  ...
  getAuthorizeUrl(redirect_uri = '', state = '', view = 'wap') {
  ...
  getAccessToken(code, redirect_uri = '', state = '', view = 'wap') {
    return new Promise((resolve, reject) => {
      request({
        method: 'POST',
        url: url.resolve(this.OAUTH_URL, '/token'),
        json: true,
        form: {
        ...
        if (body) {
          if (body.error) {
            const e = new Error(JSON.stringify(body));
            reject(e);
          }
          resolve(body);
        }
